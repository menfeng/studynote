...
用户所能操作的只是Query
Query中有Query_base
Query_base分为各种类型，最基础的为WordQuery
任何一个复合查询会调用其左查询和右查询，最后都会落实到WordQuery上的查询,WordQuery调用TextQuery
*/
/***************************Query_base******************************/
...
class Query {
	...
	friend Query operator&(const Query&, const Query&);
public:
	Query(const std::string&); // builds a new WordQuery，最基本的是创建一个WordQuery
